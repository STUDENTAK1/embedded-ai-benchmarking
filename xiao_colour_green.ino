...
#define RUN_NUMBER       1        // 1=static, 2=notarget, 3=dynamic
#define TARGET_COLOUR    2        // 1=RED, 2=GREEN, 3=BLUE
...
static inline bool isTarget(uint16_t p) {
  ...
  const int DOMINANCE = 85;
  if (TARGET_COLOUR == 1) return (R > 100) && (R > G + DOMINANCE) && (R > B + DOMINANCE);
  if (TARGET_COLOUR == 2) return (G > 100) && (G > R + DOMINANCE) && (G > B + DOMINANCE);
  return (B > 100) && (B > R + DOMINANCE) && (B > G + DOMINANCE);
}
...
void setup() {
  ...
  if (esp_camera_init(&config) != ESP_OK) {
    while (1) { Serial.println("Camera Init Error"); delay(1000); }
  }
...
void loop() {
  ...
  if (hits > 120 && xmax >= 0) {
    ...
    if (tmp_w >= 12 && tmp_h >= 12 && tmp_w < (W * 0.8) && tmp_h < (H * 0.8)) {
      det_count = 1;
