...
void setup() {
  Serial.begin(115200);
  delay(500); // NO while(!Serial) — that freezes everything
...
void loop() {
  ...
  // FOMO object detection output — same as face detection
  int det_count = 0;
  ...
  if (det_count == 0) {
    ...
    for (uint32_t i = 0; i < result.bounding_boxes_count; i++) {
      ...
      // Map label name to class_id number
      int class_id = 1; // default
      if      (strcmp(bb.label, "OBJECTA") == 0) class_id = 1;
      else if (strcmp(bb.label, "OBJECTB") == 0) class_id = 2;
      else if (strcmp(bb.label, "OBJECTC") == 0) class_id = 3;
...
#else
  // Pure classification output
  int best_class = -1;
  float best_conf = 0.6; // minimum confidence threshold
  ...
  int det_count = (best_class > 0) ? 1 : 0;
  int class_id  = (best_class > 0) ? best_class : -1;
...
bool ei_camera_capture(uint32_t img_width, uint32_t img_height, uint8_t *out_buf) {
  ...
  // RGB565 → RGB888 manual conversion with byte swap
  uint16_t *src = (uint16_t*)fb->buf;
  uint8_t  *dst = out_buf;
  for (int i = 0; i < EI_CAMERA_RAW_FRAME_BUFFER_COLS * EI_CAMERA_RAW_FRAME_BUFFER_ROWS; i++) {
    uint16_t px = src[i];
    px = (px >> 8) | (px << 8);
    dst[i*3 + 0] = ((px >> 11) & 0x1F) << 3;
    dst[i*3 + 1] = ((px >>  5) & 0x3F) << 2;
    dst[i*3 + 2] = ( px        & 0x1F) << 3;
  }
